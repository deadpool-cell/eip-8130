...
///
///         The sandbox wrapper is a 52-byte contract consisting of:
///
///           [7-byte sandbox header] [45-byte STATICCALL forwarder]
///
///         The sandbox header follows the EIP-8130 format:
///           Byte 0-1:  PUSH1 6      (jump offset to JUMPDEST)
///           Byte 2:    JUMP
///           Byte 3-4:  0x81 0x30    (magic "8130")
///           Byte 5:    version      (uint8, interface version)
///           Byte 6:    JUMPDEST
///
...
///         as a transparent read-only proxy. Gas metering is handled by the node
///         (nodes SHOULD enforce a configurable gas cap per the spec).
///
...
///         offsets, so the 7-byte header shift is fully accounted for.
///
...
///         this wrapper.
library SandboxLib {
    uint256 private constant _WRAPPER_SIZE = 52;
    uint256 private constant _DEPLOYMENT_HEADER_SIZE = 14;

    /// @notice Build the 52-byte sandbox wrapper runtime bytecode.
    /// @param verifier Address of the real verifier to forward calls to
    /// @param version  Sandbox interface version (0 for current)
    function wrapperBytecode(address verifier, uint8 version) internal pure returns (bytes memory) {
        return abi.encodePacked(
            // ── Sandbox header (7 bytes) ──
            hex"600656", // PUSH1 6, JUMP
            hex"8130", // magic
            bytes1(version),
            hex"5b", // JUMPDEST
            ...
            //     from 0x2B to 0x32 (+7 for the sandbox header).
            ...
            hex"6032" // PUSH1 0x32  (JUMPDEST offset = 50)
            hex"57" // JUMPI
            hex"fd" // REVERT
            hex"5b" // JUMPDEST @50
            hex"f3" // RETURN
    ...
    /// @notice Build deployment code: 14-byte deployment header + 52-byte wrapper.
    function deploymentCode(address verifier, uint8 version) internal pure returns (bytes memory) {
        bytes memory runtime = wrapperBytecode(verifier, version);
    ...
    function deploy(address verifier, uint8 version, bytes32 salt) internal returns (address deployed) {
        bytes memory code = deploymentCode(verifier, version);
        assembly {
    ...
    function computeAddress(address deployer, address verifier, uint8 version, bytes32 salt)
        internal
        pure
        returns (address)
    {
        bytes32 initCodeHash = keccak256(deploymentCode(verifier, version));
    ...
    function parseSandboxHeader(address deployed)
        internal
        view
        returns (uint8 version, address wrappedVerifier, bool valid)
    {
        bytes memory code = deployed.code;
        if (code.length < _WRAPPER_SIZE) return (0, address(0), false);

        // Validate sandbox header bytes
        if (code[0] != 0x60 || code[1] != 0x06 || code[2] != 0x56) return (0, address(0), false);
        if (code[3] != 0x81 || code[4] != 0x30) return (0, address(0), false);
        if (code[6] != 0x5B) return (0, address(0), false);

        version = uint8(code[5]);

        // Validate PUSH20 opcode at absolute offset 16
        if (code[16] != 0x73) return (0, address(0), false);

        // Extract 20-byte address starting at absolute offset 17
        assembly {
            wrappedVerifier := shr(96, mload(add(add(code, 0x20), 17)))
        }
        valid = true;
    }
}
